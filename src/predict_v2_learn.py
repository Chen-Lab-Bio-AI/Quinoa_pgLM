...
def load_tokenizer(args, model_dir, seq_tokenizer_class):
    # ../models/QuinoaAltitude_SV/gene/multi_class/lucasingle/matrix_variant/20251022202559/checkpoint-158796
    # Alphabet

    seq_subword, seq_tokenizer = None, None
    if not hasattr(args, "has_seq_encoder") or args.has_seq_encoder:
        if args.seq_subword:  #  False
        ...
        # zheli
            ...
            seq_subword = None
            seq_tokenizer = seq_tokenizer_class.from_predefined(args.seq_vocab_path)   # Alphabet.from_predefined(‘gene’)
            # 不满足
...
def load_trained_model(model_config, args, model_class, model_dirpath):
    ...
    try:
        model = model_class.from_pretrained(model_dirpath, args=args)  # LucaSingle.from_pretrained(training_args.bin)
        """
        from transformers.modeling_utils import PreTrainedModel
        from transformers.models.bert.configuration_bert import BertConfig
        
        class BertPreTrainedModel(PreTrainedModel):
            config_class = BertConfig  # 必须指定 config_class
        
        class LucaSingle(BertPreTrainedModel):
            def __init__(self, config):
                super(LucaSingle, self).__init__(config)
        
        @classmethod    # PreTrainedModel.from_pretrained
        def from_pretrained(cls, 模型路径, **kwargs):
            # Step 1: 加载配置, 模型路径/config.json
            config = cls.config_class.from_pretrained(模型路径, **kwargs)
            
            # Step 2: 用 config 初始化模型
            # 调用了 LucaSingle.__init__(self, config)
            model = cls(config)  
            
            # Step 3: 加载预训练权重, 模型路径/model.safetensors 或者 pytorch_model.bin
...
def load_model(args, model_name, model_dir):
    ...
    #  /home/quinoa/work/LucaOneTasks/models/QuinoaAltitude_SV/gene/multi_class/lucasingle/matrix_variant/20251022202559/checkpoint-158796
    ...
    seq_subword, seq_tokenizer = load_tokenizer(args, model_dir, seq_tokenizer_class)  #  None   common.alphabet.Alphabet

    # 生成一个配置对象。用于后续初始化一个 BERT 模型
    # BertConfig(/home/quinoa/work/LucaOneTasks/models/QuinoaAltitude_SV/gene/multi_class/lucasingle/matrix_variant/20251022202559/checkpoint-158796/config.json)
    model_config = config_class(**json.load(open(os.path.join(model_dir, "config.json"), "r")))

    model = load_trained_model(model_config, args, model_class, model_dir)    #config.json(没用)  training_args.bin  LucaSingle
...
def create_encoder_batch_convecter(
    ...
    # input_mode='single'
        ...
        assert model_args.seq_max_length is not None    # seq_max_length=4096
        encoder_config = {
            "llm_dirpath": model_args.llm_dirpath,
            "llm_type": model_args.llm_type,
            "llm_version": model_args.llm_version,
...
def run(
    ...
    # ../logs/QuinoaAltitude_SV/gene/multi_class/lucasingle/matrix_variant/20251022202559
    ...
    if not hasattr(model_args, "llm_type"):  # 'llm_type': 'lucaone',
        model_args.llm_type = "lucaone"
    if not hasattr(model_args, "llm_version"):  # 'llm_version': 'lucaone-gene'
        model_args.llm_version = "lucaone"
    if not hasattr(model_args, "llm_step"):  # 'llm_step': '36800000'
        model_args.llm_step = "5600000"

    # http://47.93.21.181/lucaone/TrainedCheckPoint/latest/logs/lucaone/lucaone-gene/
    # 下载到 ~/work/LucaOneTasks/llm/logs/lucaone/lucaone-gene/
    # http://47.93.21.181/lucaone/TrainedCheckPoint/latest/models/lucaone/lucaone-gene/checkpoint-step36800000
    # 下载到 ~/work/LucaOneTasks/llm/models/lucaone/lucaone-gene/checkpoint-step36800000
    ...
    model_args.llm_truncation_seq_length = llm_truncation_seq_length   # 4096
    model_args.atom_seq_max_length = None  # to do
    model_args.truncation_seq_length = model_args.seq_max_length  # 3074
    model_args.truncation_matrix_length = max(model_args.matrix_max_length, llm_truncation_seq_length)   # 3074 4096
    if emb_dir and not matrix_embedding_exists and "variant" not in input_type:   # 不满足
        unique_string = str(uuid.uuid4())
        emb_dir = os.path.join(emb_dir, "%s-%d" % (unique_string, gpu_id))
    model_args.emb_dir = emb_dir   # 结果路径/seq_embedding/
    model_args.step = step         # 36800000
    model_args.gpu_id = gpu_id
    model_args.threshold = threshold  # 0.5
    if gpu_id is None or gpu_id < 0:
        gpu_id = -1
        model_args.gpu_id = gpu_id

    # 更新
    model_args.seq_max_length = max(model_args.seq_max_length, llm_truncation_seq_length)  # 3074 -》 4096
    model_args.matrix_embedding_exists = matrix_embedding_exists            #   True ->   False
    ...
    model_args.task_type = task_type      # 'task_type': 'multi_class'
    model_args.model_type = model_type    # 'model_type': 'lucasingle'
    model_args.input_type = input_type    # 'input_type': 'matrix_variant'
    model_args.time_str = time_str        # 'time_str': '20251022202559',
    model_args.task_level_type = task_level_type   # task_level_type': 'seq_level',
    if model_args.label_filepath:   # ../dataset/QuinoaAltitude_SV/gene/multi_class/label.txt
    ...
    if global_trained_model is None or next(global_trained_model.parameters()).device != model_args.device:  # 对
        global_trained_model = None
        # # 20251022202559/checkpoint-158796/config.json ， None ， common.alphabet.Alphabet， LucaSingle
        model_config, seq_subword, seq_tokenizer, trained_model = load_model(model_args, model_type, model_dir)  # 'model_type': 'lucasingle'
    ...
    if not matrix_embedding_exists and gpu_id > -1 and input_type != "seq":   # False 、 3 、 matrix_variant
        # 先to cpu
        trained_model.to(torch.device("cpu"))
        assert model_args.emb_dir is not None    # infertest/2000/seq_embedding/
        ...
        for item in sequences:
            ...
            # input_mode='single'
                ...
                if "matrix" in input_type or "vector" in input_type:
                    if "variant" in model_args.input_type:
                        emb_seq_id = "_".join(seq_id.split("_")[1:])   # JQ1_Chr4A_35910823_35913895 -> Chr4A_35910823_35913895
                    else:
                        emb_seq_id = seq_id
                    encoder.__get_embedding__(
                        seq_id=emb_seq_id,
    ...
    # Step 3: prediction        task_level_type': 'seq_level',   'task_type': 'multi_class'
...
def create_run_args():
    ...
    parser.add_argument(
        ...
        required=True,
        choices=[
            "luca_base", "lucabase", "lucasingle",
            "lucapair1", "lucapair2",
            "lucapair_homo", "lucapair_heter", "lucapair_intrainter"
        ],
        help="the model type."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.output_attention_pooling_scores_dirpath:  # 对
    ...
    if args.output_logits_dirpath:   # 对
    ...
    if args.input_file is not None and os.path.exists(args.input_file):
        exists_ids = set()  # 结果文件中id
        exists_res = []     # 结果文件中行
        if os.path.exists(args.save_path):
            # continue
            print("save_path=%s exists." % args.save_path)
            if args.input_mode == "pair":   # input_mode='single'
        ...
        with (open(args.save_path, "w") as wfp):
            ...
            if args.ground_truth_idx is not None and args.ground_truth_idx >= 0:   # 'ground_truth_idx': 4
                header.append("ground_truth")
            writer.writerow(header)   # ['seq_id', 'seq', 'variant', 'top1_prob', 'top1_label', 'top4_probs', 'top4_labels', "ground_truth"]
            ...
            for row in reader:
                # row 前4列加入 batch_data,  第5列加入 batch_ground_truth,
